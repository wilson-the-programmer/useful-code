...
function highlightCode() {
    const editor = document.getElementById("topWindow");
    ...
    const keywords = new Set([
        ...
        "func","package","var","const","type","struct",
        "interface","range","go","defer","chan","map",
        "select","switch","case","default"
    ]);

    const parts = code.match(/\\b[A-Za-z_][A-Za-z0-9_]*\\b|\\d+|"[^"]*"|'[^']*'|\\S+|\\s+/g);
    ...
    for (let i = 0; i < parts.length; i++) {
        ...
        if (keywords.has(token)) {
            result += '<span class="keyword">' + safe + '</span>';
        }
        else if (/^\\d+$/.test(token)) {
            result += '<span class="number">' + safe + '</span>';
        }
        else if (/^["'].*["']$/.test(token)) {
            result += '<span class="string">' + safe + '</span>';
        }
        else if (parts[i + 1] && parts[i + 1].trim() === "(") {
            result += '<span class="function">' + safe + '</span>';
        }
        else if (parts[i + 1] && parts[i + 1].trim() === "=") {
            result += '<span class="variable">' + safe + '</span>';
        }
        else {
            result += safe;
        }
